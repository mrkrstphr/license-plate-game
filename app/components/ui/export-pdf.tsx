...
export function ExportPDF({ game, onClose }: ExportPDFProps) {
  ...
  async function handleExport() {
    ...
    const { svgStringToDataUrl, SVG_US_FLAG, SVG_CA_FLAG, SVG_HIGHWAY } = await import("~/lib/pdf-flags");
    ...
    // Pre-render everything in parallel
    const [usMapImg, caMapImg, highwayImg, usFlagImg, caFlagImg] = await Promise.all([
      includeUS ? buildUSMapImage(game.found)        : Promise.resolve(null),
      includeCA ? buildCAMapImage(game.found)        : Promise.resolve(null),
      svgStringToDataUrl(SVG_HIGHWAY, 800, 320),
      svgStringToDataUrl(SVG_US_FLAG, 760, 400),
      svgStringToDataUrl(SVG_CA_FLAG, 760, 400),
    ]);
    ...
    // Highway sign image left-aligned in bar
    doc.addImage(highwayImg, "PNG", M, 2, 44, 17.6);

    // Title text right of sign
    doc.setTextColor(...WHITE);
    doc.setFont("helvetica", "bold");
    doc.setFontSize(11);
    doc.text(game.name, M + 48, 11);
    doc.setFont("helvetica", "normal");
    doc.setFontSize(8);
    doc.setTextColor(200, 210, 230);
    doc.text(formatDate(game.date), M + 48, 18);
    ...
    function drawProgressBar(
      ...
      // Flag icon
      doc.addImage(flagImg, "PNG", M, yPos - 3.5, flagW, flagH);

      doc.setFont("helvetica", "bold");
      doc.setFontSize(9);
      doc.setTextColor(...GRAY);
      doc.text(label.toUpperCase(), M + flagW + 2, yPos);
    ...
    if (includeUS) {
      ...
      y = drawProgressBar(usFlagImg!, "US States", usFound, US_PLATES.length, SKY, y);
    }
    if (includeCA) {
      const caFound = game.found.filter(c => CA_PLATES.some(p => p.code === c)).length;
      y = drawProgressBar(caFlagImg!, "Canada", caFound, CA_PLATES.length, AMBER, y);
    }
    ...
    function drawMap(
      ...
      doc.addImage(flagImg, "PNG", M + 2, yPos + 1.3, 8.5, 5.6);
      doc.setFont("helvetica", "bold");
      doc.setFontSize(10);
      doc.setTextColor(...WHITE);
      doc.text(title, M + 13, yPos + 5.5);
      yPos += 10;
    ...
    if (includeUS && usMapImg) {
      y = drawMap(usMapImg, usFlagImg!, "US States", SKY, 1520, 920, y);
    }
    if (includeCA && caMapImg) {
      if (y + 10 > PH - M) { doc.addPage(); y = M; }
      y = drawMap(caMapImg, caFlagImg!, "Canadian Provinces & Territories", AMBER, 1600, 960, y);
    }
    ...
    function drawTable(
      ...
      doc.setFillColor(...accentColor);
      doc.rect(M, yStart, CW, 8, "F");
      doc.addImage(flagImg, "PNG", M + 2, yStart + 1.3, 8.5, 5.6);
      doc.setFont("helvetica", "bold");
      doc.setFontSize(10);
      doc.setTextColor(...WHITE);
      doc.text(title, M + 13, yStart + 5.5);
      yStart += 10;
    ...
    if (includeUS) {
      ...
      y = drawTable(US_PLATES, usFlagImg!, "US States", SKY, y);
    }
    if (includeCA) {
      if (y + 20 > PH - M) { doc.addPage(); y = M; }
      else y += 4;
      y = drawTable(CA_PLATES, caFlagImg!, "Canadian Provinces & Territories", AMBER, y);
    }
    ...
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-4"
      ...
      <div className="w-full max-w-sm rounded-2xl shadow-xl p-5 space-y-5 max-h-[85vh] overflow-y-auto"
        ...
        <p className="text-xs rounded-xl px-3 py-2" style={{ background: "var(--bg-muted)", color: "var(--text-muted)" }}>
          Includes map, progress bars, and full status table
        </p>
